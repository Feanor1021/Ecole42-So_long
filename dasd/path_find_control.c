...
int valid_path_check(t_game *so_long,t_flood***mapcpy)
{
    ...
    so_long->px=so_long->i;
    so_long->py=so_long->j;
    so_long->i = 0;
    so_long->j = 0;
    while(coordinate_finder(so_long,'C'))
    {
        *mapcpy=map_cpy(so_long,mapcpy,'G');
        if(!path_controller(so_long,mapcpy,so_long->px,so_long->py))
            return -1;  
...
int path_controller(t_game*s,t_flood ***mapcpy,int px, int py)
{
    ...
    {
        if(((*mapcpy)[px][py].num>(*mapcpy)[px-1][py].num)&&(*mapcpy)[px-1][py].fil_fg==1 && (*mapcpy)[px-1][py].data=='*')
            px--;
        else if(((*mapcpy)[px][py].num>(*mapcpy)[px+1][py].num)&&(*mapcpy)[px+1][py].fil_fg==1&& (*mapcpy)[px+1][py].data=='*')
            px++;
        else if(((*mapcpy)[px][py].num>(*mapcpy)[px][py-1].num)&&(*mapcpy)[px][py-1].fil_fg==1&& (*mapcpy)[px][py-1].data=='*')
            py--;
        else 
            py++;
    }
